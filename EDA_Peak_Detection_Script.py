...
from load_files_E4 import getInputLoadFile, get_user_input, getOutputPath

# Must be the same sample rate in load_files.py
SAMPLE_RATE = 1
        ...
        if peaks[i] == 1:
            ...
            while found == False and find_start > (i - start_WT * sampleRate):
                if EDA_deriv[find_start] < start_deriv:
                    ...
                    rise_time[i] = get_seconds_and_microseconds(pd.to_datetime(data.index[i]) -
                                                                peak_start_times[i])
        ...
        if peaks[i] == 1:
            ...
            while found == False and find_end < (i + end_WT * sampleRate) and find_end < len(peaks):
                if data['EDA'].iloc[find_end] < half_amp:
                    ...
                    decay_time[i] = get_seconds_and_microseconds(pd.to_datetime(peak_end_times[i]) -
                                                                 data.index[i])
                    ...
                    while found_rise == False:
                        if data['EDA'].iloc[find_rise] < half_amp:
                            ...
                            SCR_width[i] = get_seconds_and_microseconds(pd.to_datetime(peak_end_times[i]) -
...
def calcPeakFeatures(data,offset,thresh,start_WT,end_WT):
...
def getEDAfeatures(inputfile,outfile,settings_dict):
    data, filepath_confirm = getInputLoadFile(inputfile)
    fullData, peakData = calcPeakFeatures(data, settings_dict['offset'], settings_dict['threshold']
                                , settings_dict['rise time'], settings_dict['decay time'])
    return fullData, peakData
...
if __name__ == "__main__":
    ...
    settings_dict = {'threshold': thresh,
                     ...
                     'rise time': start_WT,
                     'decay time': end_WT}
    input = get_user_input('\tE4 Directory root: ')
    # To save output, change getEDAfeatures
    outfile = ''
    fullData,peakData = getEDAfeatures(input,outfile,settings_dict)
    print("")
